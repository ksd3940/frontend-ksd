...
//  : 예) 사용자가 버튼을 클릭했을 때 'alert창이 실행'되도록 작성
...
//  - btn.ondblclick: 오직 더블클릭 되었을 때만  실행
...
//  - btn.onmouseover, btn.onmouseout : 각가 마우스 포인터가 버튼 위에 올라가 있을 때, 포인터가 버튼에서 벗어났을 땨 실행
...
function bgChange() {
  ...
  function random(number) {
    ...
    // Math.random(): 0이상 1미만의 부동 소수점 난수 (랜덤 숫자)를 변환
...
//  첫 번째 매개변수: 이벤트 유형(등록하고자 하는 이벤트의 이름 / 예 - click, load, input등)
